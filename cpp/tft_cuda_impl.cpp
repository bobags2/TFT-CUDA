...
void quantile_heads_forward(
    ...
    // Support 2D input (B, D) by treating T=1 logically
    bool two_dim = (input.dim() == 2);
    if (two_dim) {
        TORCH_CHECK(T == 1, "For 2D input, T must be 1");
    }

    // Kernel uses FP32 internally
    ...
    // Ensure we have a float32 output buffer for the kernel
...
void quantile_heads_backward(
    ...
    // Support 2D case (B, D) by setting T=1
    bool two_dim = (input.dim() == 2 || (B > 0 && T == 1));

    // Use linear_backward kernel to get dL_dW and dL_db
    ...
    // Reshape to (M, K/N) shapes expected by linear_backward_mp: M=B*T, K=D, N=Q
    int M = B * T;
    ...
    // Compute dL_dinput = grad_output @ W^T in FP32 using ATen (highly optimized GEMM)
...
// ====================== LSTM (placeholder to force Python fallback) ======================
void lstm_variable_selection_forward(
    ...
    // The custom LSTM CUDA kernel is not yet wired to match the Python autograd interface.
    // Raise to trigger the safe PyTorch fallback path in python/tft_model.py.
